// agente-whatsapp/index.ts — v2 optimizado, una sola llamada a Groq
...
${restaurante.cobrar_senal ? `- Señal: ${restaurante.importe_senal_por_pax}€ por persona` : ""}
- Hoy: ${diaHoy} (${fechaHoy})
...
Deno.serve(async (req) => {
  ...
  if (type === "RESERVA" && data.fecha && data.minutos_reserva !== undefined && data.personas && data.nombre) {
    ...
    if (disp.status === "disponible") {
      // Confirmar con minutos del turno correcto
